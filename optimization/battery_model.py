...
def build_ampl_model(d):
    ...
    ampl=AMPL()
    ampl.read("optimization/battery_lp.mod")
    ampl.read_data("optimization/batt_data.dat")
    T=[]
    T_td=[]
    T_CHP=[]
    for t in d.T_e:
...
def update_ampl_solution_heat(d,ampl):
    ...
    # var xd{M} >= 0;
    # var xr{R} >= 0;
    # var pts_e{t in T_e, h in H} >= 0 , <= prod_remain_e[t,h]; 
    # var pts_th{t in T_th, h in H} >= 0 , <= prod_remain_th[t,h]; 
    # var xtp{t in T_th, h in H} >= 0 , <= prod_remain_th[t,h]; 
